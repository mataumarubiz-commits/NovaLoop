...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await syncFreeeEntities({ admin: auth.admin, orgId: auth.orgId, targetMonth, entityType: "payout", ids })
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "failed to sync freee payouts" },
      { status: 500 }
    )
  }
}
