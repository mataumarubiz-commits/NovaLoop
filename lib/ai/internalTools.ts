...
import { hasClientSubmissionSignal, hasEditorSubmissionSignal } from "@/lib/contentWorkflow"
...
export async function runInternalTool(params: {
  ...
  switch (tool) {
    ...
    case "get_org_dashboard_summary": {
      const { data } = await admin
        .from("contents")
        .select("id, due_client_at, due_editor_at, status, editor_submitted_at, client_submitted_at")
        .eq("org_id", actor.orgId)
      ...
      const delayed = rows.filter(
        (row) =>
          ...
          !hasClientSubmissionSignal(row.status, row.client_submitted_at ?? null)
      ).length
